              ...
              <Box display="flex" flexDirection={{ xs: 'column', md: 'row' }} gap={5} mb={6}>
                ...
                <Box flex={2} sx={{ width: '100%' }}>
                  ...
                  <Box display="flex" alignItems="center" gap={1} mb={2}>
                    ...
                    <Typography variant="body2">({product.rating?.count} reviews)</Typography>
                    <Chip label="In Stock" color="success" size="small" />
                  </Box>
                  <Typography variant="h4" color="primary" fontWeight={700} mb={2}>${product.price}</Typography>
                  ...
                  <Box display="flex" alignItems="center" gap={3}>
                    <Box display="flex" alignItems="center" border="1px solid #e0e0e0" borderRadius={2} px={1}>
                      <IconButton onClick={() => setQuantity((q) => Math.max(0, q - 1))} disabled={quantity === 0}><Remove /></IconButton>
                      <Typography mx={2} fontWeight={600}>{quantity}</Typography>
                      <IconButton onClick={() => setQuantity((q) => q + 1)}><Add /></IconButton>
                    </Box>
                    <Button
                      variant="contained"
                      color="primary"
                    ...
                    {relatedProducts.map((item) => (
                      ...
                      >
                        ...
                        <Box display="flex" alignItems="center" mb={1}>
                          <Rating value={item.rating?.rate || 0} readOnly size="small" />
                          <Typography variant="body2" color="text.secondary" ml={0.5}>({item.rating?.count})</Typography>
                        </Box>
